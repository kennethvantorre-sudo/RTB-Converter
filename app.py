...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("Operationele Tool v4.7 - Strabag Excel Support")
...
def strabag_excel_naar_railcube(excel_file):
    try:
        ...
        if not target_sheet:
            st.error("❌ Kon het tabblad 'Wagenliste' niet vinden in de Excel!")
            return pd.DataFrame()
            
        df_raw = xl.parse(sheet_name=target_sheet, header=None)
        wagons = []
        
        for idx, row in df_raw.iterrows():
            val_col_0 = str(row[0]).strip() if pd.notna(row[0]) else ""
            
            # De data begint exact waar kolom A een volgnummer bevat, én kolom B niet leeg is
            if val_col_0.isdigit() and pd.notna(row[1]):
                try:
                    # Smijt spaties weg en herstel voorloopnullen voor de 5 wagon-kolommen (index 1 t/m 5)
                    ...
                    # Assen bepalen (default naar 4 indien leeg)
                    if pd.notna(row[7]) and str(row[7]).split('.')[0].isdigit():
                        assen = int(str(row[7]).split('.')[0])
                    else:
                        assen = 4
                        
                    # Uitlezen gewichten en lengtes uit de juiste kolommen (9, 10, 11, 12, 14)
                    ...
                    remgewicht = float(str(row[14]).replace(',', '.').strip()) if pd.notna(row[14]) else 0.0
...
with col_main:
    ...
    # Strabag veilig toegevoegd als Excel-optie!
    ...
    # Dynamisch wisselen tussen PDF en Excel filters
...
# 🎨 4. SFEERBEELD (Uw originele layout-blok, exact behouden!)
if not upped:
    st.markdown("---")
